...
const songActive = document.querySelector('.song__album--item')
let timer;
var ismusic = 'VN';
isrepeat = true;
israndom = true;
let recentList = [];
function handleAudioActive(){
    if(audio = true){
        console.log(Songs[ismusic][indexSongs])
    }
}
setInterval(handleAudioActive,1000)
...
function RenderNextSongMini(indexSongs){
   ...
   RenderMiniSongUnder(indexSongs);
    Btnmini.classList.remove('hide');
    MiniBtnPlayUnder.classList.add('showminiSong');
    Btnmini.removeEventListener('click',playPause);
//   console.log(indexSongs);
}